...
async def backup_instance(public_ip, ctx):
    with paramiko.SSHClient() as ssh:
        ...
        ssh.connect(hostname=public_ip, username='ec2-user', key_filename=os.getenv('KEY_FILENAME'))

        stdin, stdout, stderr = ssh.exec_command('bash /home/ec2-user/backup-server.sh')
        output = stdout.readlines()
        exit_status = stdout.channel.recv_exit_status()

        if exit_status == 0:
            await ctx.send('Backup do servidor executado com `sucesso`!')
        else:
            print(output)
            await ctx.send('Backup do servidor executado com `falha`!')
  ...
  if server_state == 'stopped':
    ...
    if '--force' in ctx.message.content:
      ...
      if mstat.current_players > 0:
        ...
        ec2.stop_instances(InstanceIds=[instance_id])
      await ctx.send('Servidor do Minecraft desligado com sucesso!')
    else:
      ec2.stop_instances(InstanceIds=[instance_id])
      await ctx.send('Servidor do Minecraft desligado com sucesso!')
  else:
